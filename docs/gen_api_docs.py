...
if proj_toml.exists():
    ...
    mkdocs_gen_files.log.warning('Skipping API docs because "kosh" directory is missing.')
    sys.exit()
...
for path in sorted(src_path.rglob("*.py")):
    ...
    full_doc_path = Path("source/reference_guide/api_reference", doc_path)

    parts = list(module_path.parts)

    # Ensure __init__.py's get added/parsed properly and write contents to
    # the current module's index.md
    parts = ['kosh'] + parts  # add package prefix back on
...
with mkdocs_gen_files.open("sources/reference_guide/SUMMARY.md", "w") as nav_file:
