...
class KoshTestCp(KoshTest):
    ...
    def test_file_to_file(self):
        # kosh cv --stores store1.sql --destination_stores store3.sql --source file1 --destination file2
        ...
        run_cp([file_src, ], dest_name, [self.source_prefix+db1,], [self.dest_prefix+db2,])
    ...
    def test_files_to_new_directory(self):
        # kosh mv --stores_store1.sql store2.sql--destination_stores store3.sql  --source dir1 --destination dir2
        rand = str(random.randint(0, 1000000))
        store1, db1 = self.connect()
        store2, db2 = self.connect()
    ...
    def test_copy_directory_not_existing(self):
        # kosh cp --stores_store1.sql store2.sql --destination_stores store3.sql --source dir1 --destination dir2
        rand = str(random.randint(0, 1000000))
        store1, db1 =self.connect()
        store2, db2 = self.connect()
        ...
        #   pass
        dest_name = self.dest_prefix + dest_name_orig
        run_cp([self.source_prefix+orig_dir,], dest_name, [self.source_prefix+db1,], [self.dest_prefix+db2,])
        # First let's check files are moved
        for file_src in file_src_orig_associate:
            self.assertTrue(os.path.exists(file_src))
        ...
        self.assertTrue(os.path.exists(new_paths[-1]))
        new_paths.append(os.path.join(dest_name_orig, rand+"_d2d", "sub", "file2.py"))
        self.assertTrue(os.path.exists(new_paths[-1]))
    ...
    def test_move_files_pattern_to_new_directory(self):
        ...
        run_cp([self.source_prefix+os.path.join(apath,"*.testme"), self.source_prefix+os.path.join(apath,"dir1/testing_it*testme")], dest_name, [self.source_prefix+db1,],[self.dest_prefix+db2,])
    ...
    def test_copy_from_a_store_to_another_in_new_dest_dir(self):
        # kosh cp --stores source  --destination_stores dest --source file1 file2, ... --destination dir [--dataset]
        pass
    ...
    def test_copy_dataset_from_store_to_another_remote(self):
        # kosh cp --stores source  --destination_stores user@machine:/path/to/kosh/store.sql --datasets dataset1_id  dataset2_id --destination user@machine:/path/to/destination_directory 
        pass

    # This is done in mv
    def test_reassociate_files(self):
        # kosh reassociate --store store1 --source file --destination new_file_path 
        pass
    ...
    def test_dir_and_files_to_dir(self):
        ...
        file_src_orig = [rand+"_df2d/1.py", rand+"_df2d/file2.py", rand+"_f1.py", rand+"_f2.py"]
        try:
            os.removedirs(os.path.dirname(file_src_orig[0]))
        except:
            pass
        ...
        try:
            ...
            pass
        dest_name = self.dest_prefix + dest_name_orig
        run_cp([self.source_prefix+os.path.abspath(rand+"_df2d"),] + [self.source_prefix + f for f in file_src_absolute[2:]], dest_name, [self.source_prefix+db1,], [self.dest_prefix+db2,])

        new_paths = []
        for i, file_src in enumerate(file_src_absolute):
            # Test files are moved
            self.assertTrue(os.path.exists(file_src))
            dest = os.path.abspath(os.path.join(dest_name_orig, file_src_orig[i]))
            new_paths.append(dest)
            self.assertTrue(os.path.exists(dest))
            # Test datasets are updated        
    ...
    def test_file_in_double_dir_to_file(self):
        ...
        file_src_orig = os.path.abspath(rand+"_a_dir/another_dir/file_to_file.py")
        try:
            os.remove(file_src_orig)
        except:
            pass
