...
class SlowDowner(kosh.transformers.KoshTransformer):
    ...
    def __init__(self, sleep_time=1, cache_dir="kosh_cache", cache=False):
        super(SlowDowner, self).__init__(cache_dir=cache_dir, cache=cache, sleep_time=sleep_time)
...
class TestKoshTransformers(koshbase.KoshTest):
    def testTransformers(self):
        ...
        start = time.time()
        ds.get("numbers", transformers=[Ints2Np(), SlowDowner(1), Even(), SlowDowner(2)])
        self.assertGreater(time.time() - start, 3.)

        start = time.time()
        ds.get("numbers", format="numpy", transformers=[Ints2Np(), SlowDowner(1), Even(), SlowDowner(2)])
        ...
        ds.get("numbers", format="numpy", transformers=[Ints2Np(), SlowDowner(2, cache_dir="kosh_cache", cache=True), Even(), SlowDowner(1)])
        # Still should be about same length
        self.assertGreater(time.time() - start, 3.)

        start = time.time()
        ds.get("numbers", format="numpy", transformers=[Ints2Np(), SlowDowner(2, cache_dir="kosh_cache", cache=True), Even(), SlowDowner(1, cache_dir="kosh_cache", cache=True)])
    ...
    def test_splitter(self):
        ...
        data = ds.get("range_1000", cache=False, transformers=[kosh.transformers.Splitter(n_splits=1, random_state=45),])
        ...
        data = ds.get("range_1000", cache=False, transformers=[kosh.transformers.Splitter(n_splits=1, random_state=40),])
        train, test = data[0]
        self.assertFalse(numpy.allclose(train[-5:], [544, 892, 643, 414, 971]))
        self.assertFalse(numpy.allclose(test[-5:], [418, 978, 65, 96, 286]))

        # Add validation
        data = ds.get("range_1000", cache=False, transformers=[kosh.transformers.Splitter(n_splits=1, random_state=73, test_size=.15, train_size=.75, validation_size=.1),])
        self.assertEqual(len(data), 1)
        data = data[0]
