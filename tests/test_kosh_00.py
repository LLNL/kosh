...
class KoshTest(unittest.TestCase):
    ...
    def test_connect(self):
        self.init_db()
        if self.engine == "sina":
            store = KoshStore(engine="sina", username=os.getlogin(), sql='sql', db_path=kosh_test_sql_file)
    ...
    def init_db(self):
        if self.engine == "sina":
            ...
            cmd = "{}/bin/python scripts/init_sina.py --sina_db={}".format(sys.prefix, kosh_test_sql_file)
            print("EXEC:", cmd)
