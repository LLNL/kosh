...
class KoshTestStore(KoshTest):
    ...
    def test_kosh_gpu_local(self):
        # Make sure everything works before we scale it up

        # Script path
        script_path = os.path.join(self.test_pth, "kosh_gpu.py")

        # Without lock-strategy single run
        # rm -rf run_0*
        # python tests/kosh_gpu.py -s run_0.sql --run-number 0 --datasets 5 --ensembles 5 --lock-strategy None
        ...
        # rm -rf run_0*
        # python tests/kosh_gpu.py -s run_0.sql --run-number 0 --datasets 5 --ensembles 5 --lock-strategy RFileLock
