...
class KoshTestDataset(KoshTest):
    def test_kosh_command(self):
        ...
        store, kosh_db = self.connect(db_uri="cmd_line.sql", dataset_record_type="obs")
        ...
        o, e = run_cmd("kosh search -s 'cmd_line.sql' -d obs PARAM1=DataRange(140,445)")
        self.assertEqual(len(o), 8)
        o, e = run_cmd("kosh add -s 'cmd_line.sql' -d obs -i '2020-03-11-13-45-23' PARAM1=156 PARAM2=.2 PARAM3=something")
        ...
        o, e = run_cmd("kosh remove -s 'cmd_line.sql' -d obs -i '2020-03-11-13-45-23' -f")
        with self.assertRaises(Exception) as err:
            ds = store.open('2020-03-11-13-45-23')
        o, e = run_cmd("kosh associate -s 'cmd_line.sql' -d obs -i '2019-04-05-02-11-29' -u tests/baselines/mash/node_extracts2/node_extracts2.hdf5 -m hdf5")
        ds = store.open('2019-04-05-02-11-29')
        self.assertEqual(len(ds._associated_data_), 1)
        o, e = run_cmd("kosh deassociate -s 'cmd_line.sql' -d obs -i '2019-04-05-02-11-29' -u tests/baselines/mash/node_extracts2/node_extracts2.hdf5")
        self.assertEqual(len(ds._associated_data_), 0)
        o, e = run_cmd("kosh associate -s 'cmd_line.sql' -d obs -i '2019-04-05-02-11-29' -u tests/baselines/mash/node_extracts2/node_extracts2.hdf5 -m hdf5")
        o, e = run_cmd("kosh features -s 'cmd_line.sql' -d obs -i '2019-04-05-02-11-29'")
